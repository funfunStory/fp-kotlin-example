...
//     AJust(10).fmap(product)     // compile error

    val curriedProduct: (Int) -> (Int) -> Int = product.curried()
    val maybeProductTen: Maybe<(Int) -> Int> = Just(10).fmap(curriedProduct)

    println(maybeProductTen.fmap { it(5) })   // AJust(50)
    println(maybeProductTen.fmap { f: (Int) -> Int -> f(5) })   // AJust(50)

//    maybeProductTen.fmap { it(AJust(5)) }  // compile error
}
